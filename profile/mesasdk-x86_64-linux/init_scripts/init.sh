...
if [ ! -f "${MESASDK_ROOT}/etc/check_preq.done" ]; then
	...
	echo "Please visit http://user.astro.wisc.edu/~townsend/static.php?ref=mesasdk#Prerequisites_2 and review the prerequisites required for use of the MESA SDK"
	popd > /dev/null
	return 1
    fi
    rm config.log
